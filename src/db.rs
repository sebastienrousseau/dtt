...
#[cfg(feature = "db-sqlx")]
use sqlx::{decode::Decode, encode::{Encode, IsNull}, postgres::{PgArgumentBuffer, PgTypeInfo, PgValueRef, Postgres}, Type};
...
impl<'q> Encode<'q, Postgres> for DateTime {
    fn encode_by_ref(&self, buf: &mut PgArgumentBuffer) -> Result<IsNull, Box<dyn std::error::Error + Send + Sync>> {
        let odt = time::OffsetDateTime::from_unix_timestamp_nanos((self.unix_seconds as i128 * 1_000_000_000) + self.nanoseconds as i128).unwrap_or_else(|_| time::OffsetDateTime::now_utc()).to_offset(self.offset());
        <time::OffsetDateTime as Encode<'q, Postgres>>::encode_by_ref(&odt, buf)
    }
}
...
impl<'r> Decode<'r, Postgres> for DateTime {
    fn decode(value: PgValueRef<'r>) -> Result<Self, Box<dyn std::error::Error + 'static + Send + Sync>> {
        let odt = <time::OffsetDateTime as Decode<'r, Postgres>>::decode(value)?;
        Ok(Self { unix_seconds: odt.unix_timestamp(), nanoseconds: odt.nanosecond(), utc_offset_minutes: (odt.offset().whole_seconds() / 60) as i16 })
    }
}
...
use postgres_types::{FromSql as PgFromSql, IsNull as PgIsNull, ToSql as PgToSql, Type as PgType};
#[cfg(feature = "db-postgres")]
use bytes::BytesMut;
...
impl<'a> PgFromSql<'a> for DateTime {
    fn from_sql(ty: &PgType, raw: &'a [u8]) -> Result<Self, Box<dyn std::error::Error + Sync + Send>> {
        let odt = <time::OffsetDateTime as PgFromSql>::from_sql(ty, raw)?;
        Ok(Self {
            unix_seconds: odt.unix_timestamp(),
            nanoseconds: odt.nanosecond(),
            utc_offset_minutes: (odt.offset().whole_seconds() / 60) as i16,
...
impl PgToSql for DateTime {
    fn to_sql(&self, ty: &PgType, out: &mut BytesMut) -> Result<PgIsNull, Box<dyn std::error::Error + Sync + Send>> {
        let odt = time::OffsetDateTime::from_unix_timestamp_nanos((self.unix_seconds as i128 * 1_000_000_000) + self.nanoseconds as i128).unwrap_or_else(|_| time::OffsetDateTime::now_utc()).to_offset(self.offset());
