...
mod tests {
    ...
    mod low_level_tests {
        ...
        fn test_error_serialization_exhaustive() {
            let parse_err = time::PrimitiveDateTime::parse(
                "bad",
                &time::format_description::well_known::Rfc3339,
            )
            .unwrap_err()
            .into();
            let comp_err: time::error::ComponentRange =
                time::Time::from_hms(25, 0, 0).unwrap_err().into();
            ...
            let errs = vec![
                ...
                (
                    DateTimeError::ParseError(parse_err),
                    "\"ParseError\"",
                ),
                (
                    DateTimeError::ComponentRange(comp_err),
                    "\"ComponentRange\"",
                ),
            ];
